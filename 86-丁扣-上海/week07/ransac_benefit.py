...
def ransac(data, model, n, k, t, d, debug=False, return_all=False, retry=10):
    """
    输入:
        data - 样本点
        model - 假设模型:事先自己确定
    ...
    try:
        ...
        while iterations < k:
            maybe_idxs, test_idxs = random_partition(n, data.shape[0])  # 从data中随机获取n个数据节点的索引，分为内群索引和外群索引
            # print('test_idxs = ', test_idxs)
            maybe_inliers = data[maybe_idxs, :]  # 获取size(maybe_idxs)行数据(Xi,Yi) --->内群点
            test_points = data[test_idxs]  # 若干行(Xi,Yi)数据点 --->外群点
            ...
            if (len(also_inliers) > d):
                betterdata = np.concatenate((maybe_inliers, also_inliers))  # 样本连接，将随机抽取的内群点与满足要求的外群点合并
                ...
                if thiserr < besterr:
                    bestfit = bettermodel
                    besterr = thiserr
                    best_inlier_idxs = np.concatenate((maybe_idxs, also_idxs))  # 更新局内点,将新点加入 remarks: 满足要求的内群和外群索引合并
            iterations += 1
        if bestfit is None:
        ...
        retry -= 1
        if retry > 0:
            print(f'---没有得到最优模型解，正在重试，剩余{retry}次...')
            return ransac(data, model, n, k, t, d, debug=debug, return_all=return_all, retry=retry)
        else:
            raise e
...
def test():
    ...
    if 1:
        ...
        A_col0_sorted = A_exact[sort_idxs]  # 秩为2的数组  todo 也可以写成 np.sort(A_exact, axis=0)
        ...
        pylab.plot(A_col0_sorted[:, 0],
                   np.dot(A_col0_sorted, ransac_fit)[:, 0], 'r',
                   label='RANSAC fit')
        pylab.plot(A_col0_sorted[:, 0],
