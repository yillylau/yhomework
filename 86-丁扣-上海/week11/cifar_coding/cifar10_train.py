...
images_test, labels_test = inputs(data_dir=data_dir, batch_size=batch_size, distorted=False)
# 创建x和y_两个placeholder，用于在训练或评估时提供输入的数据和对应的标签值。
# 要注意的是，由于以后定义全连接网络的时候用到了batch_size，所以x中，第一个参数不应该是None，而应该是batch_size
x = tf.compat.v1.placeholder(tf.float32, [batch_size, 24, 24, 3])
_y = tf.compat.v1.placeholder(tf.int32, [batch_size])
...
# todo 需要注意这个dim其实就是下一步输入的每个图片的数据个数数值，因为是拍扁了，行不变，列增多
# 建立第一个全连接层
...
# 建立第二个全连接层
weight2 = variable_with_weight_loss(shape=[384, 192], stddev=0.04, w1=0.04)
...
cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=result, labels=tf.cast(_y, tf.int64))

weights_with_l2_loss = tf.add_n(tf.compat.v1.get_collection("losses"))
...
top_k_op = tf.nn.in_top_k(result, _y, 1)
...
with tf.compat.v1.Session() as sess:
    ...
    for step in range(max_steps):
        ...
        _, loss_value = sess.run([train_op, loss], feed_dict={x: image_batch, _y: label_batch})
        duration = time.time() - start_time  # 训练一个batch数据所花费的时间
        if step % 100 == 0:
    ...
    for j in range(num_batch):
        image_test_batch, label_test_batch = sess.run([images_test, labels_test])
        predictions = sess.run([top_k_op], feed_dict={x: image_test_batch, _y: label_test_batch})
