...
def calc_iou(R, config, all_boxes, width, height, num_classes):
    ...
    y_class_regr_label = []
    IoUs = []
    # print(gta)    遍历所有R，把所有的较好的先验框映射到特征图谱
    for ix in range(R.shape[0]):
        ...
        # 如果有这种先验框
            ...
            if config.classifier_min_overlap <= best_iou < config.classifier_max_overlap:
                ...
                raise RuntimeError
        # print(label)
        class_label = num_classes * [0]  # (21, 1)
        # 类似于one-hot
        class_label[label] = 1
        ...
        # 当不是背景框的时候
        if label != -1:
            # 标签存放在数组上的位置，其实不存放标签，只用存放位置偏移信息
            label_pos = 4 * label
