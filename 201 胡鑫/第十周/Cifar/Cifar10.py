...
pool2 = tf.nn.max_pool(relu2, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding="SAME")

# 为了进行与全连接层相连，这里有个拍扁的操作，将pool2输出变成一维向量，并使用get_shape()获取扁平化之后的长度
...
with tf.Session() as sess:
    sess.run(init_op)
    # 启动线程操作，这是因为之前数据增强的时候使用train.shuffle_batch()函数的时候
    # 通过参数num_threads()配置了16个线程用于组织batch的操作
    tf.train.start_queue_runners(coord=coord)

    # 每隔100step会计算并展示当前loss、每秒钟能训练的样本数量、以及训练一个batch数据所花费的时间
