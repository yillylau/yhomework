...
# 本文件用于生成anchor，anchor是一种候选框，用于检测物体
config = Config()

def generate_anchors(sizes=None, ratios=None):                                          # 生成anchor
    if sizes is None:
    ...
    num_anchors = len(sizes) * len(ratios)                                              # 一共有9种anchor

    anchors = np.zeros((num_anchors, 4))                                                # 9行4列的矩阵，每一行代表一个anchor，4个值分别代表x1,y1,x2,y2
