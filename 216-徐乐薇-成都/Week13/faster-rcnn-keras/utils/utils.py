...
# 本文件用于定义一些工具函数，包括计算iou，计算先验框，计算真实框与先验框的偏移量，计算真实框与先验框的iou，计算真实框与先验框的类别

class BBoxUtility(object):                                                          # 用于计算真实框与先验框的iou
...
# 该方法用于将预测结果解码成真实框的预测结果，即将预测结果转化成在原图上的位置，以及宽与高
    ...
    def detection_out(self, predictions, mbox_priorbox, num_classes, keep_top_k=300,        # 保留的先验框的数量，保留300个。对每个特征层的预测结果进行处理，第一个特征层对应的是conv4_3，以此类推
