...
def main():
    ...
    output_path = ""
    data_path = path.join(output_path, data_img_name) # 图片路径 例如：'lenna.png' 或者 'D:/lenna.png'
    ...
    save_img(img_light, "%s_light.png" % data_img_name.split(".")[0], output_path) # 保存图片 例如：'lenna_light.jpg'
    save_img(img_resize, "%s_resize.png" % data_img_name.split(".")[0], output_path)
    save_img(img_contrast, "%s_contrast.png" % data_img_name.split(".")[0], output_path)
    save_img(img_sharp, "%s_sharp.png" % data_img_name.split(".")[0], output_path)
    save_img(img_blur, "%s_blur.png" % data_img_name.split(".")[0], output_path)
    # save_img(img_rotate, "%s_rotate.jpg" % data_img_name.split(".")[0], output_path)
    # 色度增强
    img_color.save(path.join(output_path, "%s_color.png" % data_img_name.split(".")[0]))
    img_rotate1.save(path.join(output_path, "%s_rotate.png" % data_img_name.split(".")[0]))
