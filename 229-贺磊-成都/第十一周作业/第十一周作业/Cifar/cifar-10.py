...
def variable_with_weight_loss(shape, stddev, w1):
    ...
    :param shape: shape定义了要创建的变量的形状
    :param stddev: stddev定义了创建随机张量时的标准差
    :param w1: wl表示权重衰减的比例。
    :return: 函数返回创建的变量var
    """
    #  函数通过`tf.Variable(tf.truncated_normal(shape,  stddev=stddev))
    #  创建一个名为var的变量，它是一个截断正态分布的张量，形状和标准差分别为shape和stddev。
...
# 创建第三个卷积层
kernel3 = variable_with_weight_loss(shape=[5, 5, 64, 128], stddev=5e-2, w1=0.0)
...
reshape = tf.reshape(pool3, [batch_size, -1])  # 这里面的-1代表将pool2的三维结构拉直为一维结构
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            ...
            print("epoch%d,loss=%.2f(%.1f examples/sec;%.3f sec/batch)" % (
                epoch, loss_value, examples_per_sec, sec_per_batch))
