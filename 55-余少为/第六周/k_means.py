...
def k_means(src, k, attempts):
    ...
    while flg and count < attempts:
        ...
        for i in range(k):
            ...
            for j in range(len(lab[i])):
                vsum += data[lab[i][j]]     # 各个簇类像素值求和
            _p.append(int(vsum / len(lab[i])))  # 计算均值，当作新的质心
    ...
    for i in range(k):
        print(len(lab[i]))
        for j in range(len(lab[i])):
            data[lab[i][j]] = p[i]      # 用最终的质心替换对应簇类的像素值

    return data.reshape(h, w).astype(np.uint8)
