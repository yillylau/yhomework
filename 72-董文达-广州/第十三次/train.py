...
from nets.frcnn_trainning import cls_loss, smooth_l1, Generator, get_img_output_length, class_loss_cls, class_loss_regr
...
if __name__ == '__main__':
    ...
    for i in range(EPOCH):
        ...
        while True:
            ...
            losses[item_num, 0] = loss_rpn[1]
            losses[item_num, 1] = loss_rpn[2]
            losses[item_num, 2] = loss_class[1]
            losses[item_num, 3] = loss_class[2]
            losses[item_num, 4] = loss_class[3]

            train_step += 1
            item_num += 1
            progbar.update(item_num, [('rpn_cls', np.mean(losses[:item_num, 0])), ('rpn_regr', np.mean(losses[:item_num, 1])),
                                      ('detector_cls', np.mean(losses[:item_num, 2])), ('detector_regr', np.mean(losses[:item_num, 3]))])

            if item_num == EPOCH_LENGTH:
