...
from keras.layers import Dense, Activation, Conv2D, MaxPooling2D, AveragePooling2D, Input, BatchNormalization
...
def InceptionV3(input_shape=[299, 299, 3], classes=1000):
    ...
    branchpool = conv2d_bn(branchpool, 32, 1, 1)
    ...
    branchpool = AveragePooling2D((3, 3), strides=(1, 1), padding='same')(x)
    branchpool = conv2d_bn(branchpool, 64, 1, 1)

    x = layers.concatenate([branch1x1, branch5x5, branch3x3dbl, branchpool], axis=3, name='mixed1')
    ...
    x = layers.concatenate([branch1x1, branch5x5, branch3x3dbl, branchpool], axis=3, name='mixed2')
    ...
    branchpool = MaxPooling2D((3, 3), strides=(2, 2))(x)
    ...
    branchpool = AveragePooling2D((3, 3), strides=(1, 1), padding='same')(x)
    branchpool = conv2d_bn(branchpool, 192, 1, 1)
    ...
    for i in range(2):
        ...
        x = layers.concatenate([branch1x1, branch7x7, branch7x7dbl, branchpool], axis=3, name='mixed' + str(5 + i))
    ...
    for i in range(2):
        ...
        x = layers.concatenate([branch1x1, branch3x3, branch3x3dbl, branchpool], axis=3, name='mixed'+str(9+i))
