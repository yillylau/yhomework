...
class yolo_predictor:
    ...
    def eval(self, yolo_outputs, image_shape, max_boxes=20):
        ...
        for i in range(len(yolo_outputs)):
            _boxes, _boxes_scores = self.boxes_and_scores(yolo_outputs[i], self.anchors[anchor_mask[i]], len(self.classes_names), input_shape, image_shape)
            boxes.append(_boxes)
