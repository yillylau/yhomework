...
data_dir = "Cifa10_dataset"


def variable_with_weight_loss(shape, stddev, w1):
    '''
    使用参数w1控制l2loss的大小，使用tf.nn.l2_loss来计算权重，然后通过累加计算总loss
    :param shape:
    :param stddev:
    :param w1:
    :return:
    '''
    # stddev用于控制生成的正态分布的标准差
...
weight2 = variable_with_weight_loss(shape=[384, 192], stddev=0.03, w1=0.004)
fc_bias2 = tf.Variable(tf.constant(0.1, shape=[192]))
local_4 = tf.nn.relu(tf.matmul(fc_1, weight2) + fc_bias2)

weight3 = variable_with_weight_loss(shape=[192, 10], stddev=1 / 192, w1=0.0)
fc_bias3 = tf.Variable(tf.constant(0.1, shape=[10]))
result = tf.add(tf.matmul(local_4, weight3), fc_bias3)
...
with tf.Session() as sess:
    ...
    for step in range(max_steps):
        ...
        if step % 100 == 0:
            num_examples_per_sec = batch_size / duration
            sec_per_batch = float(duration)
            print("step %d ,loss=%.2f(%.1f examples/sec; %.3f sec/batch)" % (
                step, loss_value, num_examples_per_sec, sec_per_batch))
