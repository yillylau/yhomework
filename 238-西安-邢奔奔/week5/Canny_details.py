#!/usr/bin/python
# -*- coding:utf-8 -*-

import numpy as np
import matplotlib.pyplot as plt
import math

if __name__ =='__main__':
    img_path = ('/Users/aragaki/artificial/image/lenna.png')
    img = plt.imread(img_path)
    if img_path[-4:] == '.png':
        img = img * 255
    img = img.mean(axis=-1)


    #   高斯平滑
    sigma = 0.5
    dim = int(np.round(6 * sigma + 1))
    if dim % 2 == 0:
        dim = dim + 1
    Guassian_filter = np.zeros([dim,dim])
    tmp = [i-dim//2 for i in range(dim)]
    n1 = 1/(2*math.pi*sigma**2)
    n2 = -1/(2*sigma**2)
    for i in range(dim):
        for j in range(dim):
            Guassian_filter[i,j] = n1*math.exp(n2*(tmp[i]**2+tmp[j]**2))
    Guassian_filter = Guassian_filter / Guassian_filter.sum()
    img_new = np.zeros(img.shape)
    dx,dy = img.shape
    img_pad = np.pad(img,((dim//2,dim//2),(dim//2,dim//2)),'constant')
    for i in range(dx):
        for j in range(dy):
            img_new[i,j] = np.sum(img_pad[i:i+dim,j:j+dim] * Guassian_filter)
    plt.figure(1)
    plt.imshow(img_new.astype(np.uint8),cmap='gray')
    plt.axis('off')


    #   计算梯度
    soble_kernel_x = np.array([[-1,0,1],[-2,0,2],[-1,0,1]])
    soble_kernel_y = np.array([[1,2,1],[0,0,0],[-1,-2,-1]])
    img_tidu_x = np.zeros(img_new.shape)
    img_tidu_y = np.zeros([dx,dy])
    img_tidu = np.zeros([dx,dy])
    for i in range(dx):
        for j in range(dy):
            img_tidu_x[i,j] = np.sum(img_pad[i:i+3,j:j+3]*soble_kernel_x)
            img_tidu_y[i,j] = np.sum(img_pad[i:i+3,j:j+3]*soble_kernel_y)
            img_tidu[i,j] = np.sqrt(img_tidu_x[i,j]**2 + img_tidu_y[i,j]**2)
    img_tidu_x[img_tidu_x == 0] = 0.00000001
    angle = img_tidu_y /img_tidu_x
    plt.figure(2)
    plt.imshow(img_tidu.astype(np.uint8),cmap='gray')
    plt.axis('off')


    #   非极大值抑制
    img_yizhi = np.zeros(img_tidu.shape)
    for i in range(1,dx-1):
        for j in range(1,dy-1):
            flag = True
            tmp = img_tidu[i-1:i+2, j-1:j+2]
            if angle[i,j] <= -1:
                num1 = (tmp[0,1] - tmp[0,0]) / angle[i,j] + tmp[0,1]
                num2 = (tmp[2,1] - tmp[2,2]) / angle[i,j] + tmp[2,1]
                if not (img_tidu[i,j] > num1 and img_tidu[i,j] > num2):
                    flag = False
            elif angle[i,j] >= 1:
                num1 = (tmp[0,2] - tmp[0,1]) / angle[i,j] + tmp[0,1]
                num2 = (tmp[2,0] - tmp[2,1]) / angle[i,j] + tmp[2,1]
                if not (img_tidu[i, j] > num1 and img_tidu[i, j] > num2):
                    flag = False
            elif angle[i,j] > 0:
                num1 = (tmp[0,2] - tmp[1,2]) * angle[i,j] + tmp[1,2]
                num2 = (tmp[2,0] - tmp[1,0]) * angle[i,j] + tmp[1,0]
                if not (img_tidu[i, j] > num1 and img_tidu[i, j] > num2):
                    flag = False
            else:
                num1 = (tmp[1,0] - tmp[0,0]) * angle[i,j] + tmp[1,2]
                num2 = (tmp[1,2] - tmp[2,2]) * angle[i,j] + tmp[2,2]
                if not (img_tidu[i, j] > num1 and img_tidu[i, j] > num2):
                    flag = False
            if flag:
                img_yizhi[i,j] = img_tidu[i,j]

    plt.figure(3)
    plt.imshow(img_yizhi.astype(np.uint8),cmap='gray')
    plt.axis('off')


    #   双阈值检测
    lower_boundary = img_tidu.mean() * 0.5
    higer_boundary = lower_boundary * 3
    zhan = []
    for i in range(1,img_yizhi.shape[0]-1):
        for j in range(1,img_yizhi.shape[1]-1):
            if img_yizhi[i,j] >= higer_boundary:
                zhan.append([i,j])
                img_yizhi[i,j] = 255
            elif img_yizhi[i,j] <= lower_boundary:
                img_yizhi[i,j] = 0
    while not len(zhan):
        tmpx,tmpy = zhan.pop();
        a = img_yizhi[tmpx-1:tmpx+2,tmpy-1:tmpy+2]
        if (a[0, 0] < high_boundary) and (a[0, 0] > lower_boundary):
            img_yizhi[tmpx - 1, tmpy - 1] = 255  # 这个像素点标记为边缘
            zhan.append([tmpx - 1, tmpy - 1])  # 进栈
        if (a[0, 1] < high_boundary) and (a[0, 1] > lower_boundary):
            img_yizhi[tmpx - 1, tmpy] = 255
            zhan.append([tmpx - 1, tmpy])
        if (a[0, 2] < high_boundary) and (a[0, 2] > lower_boundary):
            img_yizhi[tmpx - 1, tmpy + 1] = 255
            zhan.append([tmpx - 1, tmpy + 1])
        if (a[1, 0] < high_boundary) and (a[1, 0] > lower_boundary):
            img_yizhi[tmpx, tmpy - 1] = 255
            zhan.append([tmpx, tmpy - 1])
        if (a[1, 2] < high_boundary) and (a[1, 2] > lower_boundary):
            img_yizhi[tmpx, tmpy + 1] = 255
            zhan.append([tmpx, tmpy + 1])
        if (a[2, 0] < high_boundary) and (a[2, 0] > lower_boundary):
            img_yizhi[tmpx + 1, tmpy - 1] = 255
            zhan.append([tmpx + 1, tmpy - 1])
        if (a[2, 1] < high_boundary) and (a[2, 1] > lower_boundary):
            img_yizhi[tmpx + 1, tmpy] = 255
            zhan.append([tmpx + 1, tmpy])
        if (a[2, 2] < high_boundary) and (a[2, 2] > lower_boundary):
            img_yizhi[tmpx + 1, tmpy + 1] = 255
            zhan.append([tmpx + 1, tmpy + 1])
    for i in range(img_yizhi.shape[0]):
        for j in range(img_yizhi.shape[1]):
            if (img_yizhi[i,j] != 255 and img_yizhi[i,j] != 0):
                img_yizhi[i,j] = 0

    plt.figure(4)
    plt.imshow(img_yizhi.astype(np. uint8),cmap='gray')
    plt.axis('off')
    plt.show()


