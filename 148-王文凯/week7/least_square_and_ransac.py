...
def main():
    ...
    n_input = 1  # 输入变量个数
    n_output = 1  # 输出变量个数
    A_exact = 20 * np.random.random((n_samples, n_input))  # 随机生成0-20之间的500个数据:行向量
    ...
    A_noisy[outlier_idxs] = 20 * np.random.random((n_outliers, n_input))  # 加入噪声和局外点的Xi
    B_noisy[outlier_idxs] = 50 * np.random.normal(size=(n_outliers, n_output))  # 加入噪声和局外点的Yi
    ...
    debug = False
    model = linear_least_square_model(input_columns, output_columns, debug=debug)  # 类的实例化:用最小二乘生成已知模型
