...
def ransac(data, model, n, k, t, d, debug = False, return_all = False):
    ...
    while iterations < k:
        maybe_idxs, test_idxs = random_partition(n, data.shape[0])# 取data的前n个数组成数组maybe_idxs，剩余部分组成数组test_idxs
        maybe_inliers = data[maybe_idxs, :]#获取size(maybe_idxs)行数据(Xi,Yi)作为样本数据
        test_points = data[test_idxs]#若干行(Xi,Yi)数据点
        maybemodel = model.fit(maybe_inliers)#根据样本数据拟合模型，maybemodel为拟合模型的[k,b]
        test_err = model.get_error(test_points, maybemodel)#除样本数据外剩余数据计算误差:平方和最小
        also_idxs = test_idxs[test_err < t]#判断剩余数据每个点误差是否在阈值范围内
        print('also_idxs',also_idxs)
        also_inliers = data[also_idxs,:]#将误差在阈值范围内的点设为内群点
        ...
        if (len(also_inliers) > d):
            ...
            bettermodel = model.fit(betterdata)#对内群点重新拟合模型
