...
'''
计算loss需要预测框pred和真实框target对比。
预测框pred：
对于yolo3的模型来说，网络最后输出的内容就是三个特征层每个网格点对应的预测框及其种类，
即三个特征层分别对应着图片被分为不同size的网格后，每个网格点上三个先验框对应的位置、置信度及其种类。
如果使用coco数据集集，结果的shape为(batch_size,13,13,255)，(batch_size,26,26,255)，(batch_size,52,52,255)
数据处理、解码了之后才是真实图像上的情况。
...
1、判断真实框在图片中的位置，判断其属于哪一个网格点去检测。
   判断真实框和这个特征点的哪个先验框重合程度最高。计算该网格点应该有怎么样的预测结果才能获得真实框，与真实框重合度最高的先验框被用于作为正样本。
2、根据网络的预测结果获得预测框，计算预测框和所有真实框的重合程度，
    如果重合程度大于一定界限，则将该预测框对应的先验框忽略。其余作为负样本。
3、最终损失由三个部分组成：
    a、正样本，编码后的长宽与xy轴偏移量与预测值的差距。
