...
data_dir = r"./cifar_data/cifar-10-batches-bin/"
...
images_test,labels_test = Cifar10_data.inputs(data_dir,batch_size,distorted = False)
...
#创建第一个卷积层 shape=(kh,kw,ci,co)，ci 输入通道，co输出通道 
kernel1 = variable_with_weight_loss(shape=[5,5,3,64],stddev=5e-2,w1=0.0)
# conv1 = tf.nn.conv2d(x,kernel1,[1,1,1,1],padding="SMAE")
...
# pool2是一个形状为[batch_size, height, width, channels]的张量，其中batch_size表示批量大小，height表示图像高度，width表示图像宽度，channels表示图像通道数。
# 通过reshape=tf.reshape(pool2,[batch_size,-1])，reshape张量的形状将变成[batch_size, height*width*channels]，即将pool2张量的三个空间维度压缩成一个，得到一个形状为[batch_size, num_pixels]的张量，其中num_pixels=height*width*channels。
...
weights_with_l2_cross = tf.add_n(tf.get_collection("losses"))
loss = tf.reduce_mean(cross_entropy) + weights_with_l2_cross
#添加优化器
...
with tf.Session() as sess:
    ...
    for step in range(max_steps):
        ...
        if step%100 == 0:
            ...
            print("step %d,loss=%0.2f (%0.1f examples/sec;%0.3f sec/batch )"%(step,loss_value,examples_per_sec,sec_per_batch))
#计算最终的正确率
    ...
    #打印正确率信息
    # print("accuracy is % .3f%%"%((true_count/total_sample_count)*100))
