...
def read_cifar10(filequeue):
    ...
    result.key,value = reader.read(filequeue)                      #使用该类的read()函数从文件队列里面读取文件
    ...
    #这一步是转换数据排布方式，变为(h,w,c) [1,2,0]是列号
    result.uint8image = tf.transpose(depth_major,[1,2,0])
    #返回值是已经把目标文件里面的信息都读取出来
    return result

def inputs(data_dir,batch_size,distorted):  #这个函数就distorted对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    filenames = [os.path.join(data_dir,"data_batch_%d.bin"%i) for i in range(1,6)] #拼接地址
    filequeue = tf.train.string_input_producer(filenames)                          #根据已经有的文件地址创建一个文件队列
    read_input = read_cifar10(filequeue)
