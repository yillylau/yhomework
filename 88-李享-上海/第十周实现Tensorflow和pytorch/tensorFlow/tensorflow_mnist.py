...
if not os.path.exists(dataset_dir):
    ...
    '''
    input_data.read_data_sets(dataset_dir, one_hot=True) 是 TensorFlow 提供的函数，用于下载和读取 MNIST 数据集。MNIST 是一个包含手写数字图像的经典数据集，常用于测试机器学习和深度学习模型。

dataset_dir：表示数据集下载的目录。如果该目录不存在，函数会自动创建。
one_hot=True：表示使用 One-Hot 编码来表示标签。在 One-Hot 编码中，每个类别的标签被表示为一个只有一个元素为 1，其余元素为 0 的向量。
'''
mnist = input_data.read_data_sets(dataset_dir, one_hot=True)

x = tf.compat.v1.placeholder(tf.float32, shape=[None, 28*28])#输入
y_true = tf.compat.v1.placeholder(tf.float32, shape=[None, 10])#标签
...
def weight_variable(shape):
  initial = tf.compat.v1.truncated_normal(shape, stddev=0.1)#truncated_normal生成符合正太分布的随机张量
...
def max_pool_2x2(x):
  return tf.nn.max_pool(x, ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1], padding='SAME')  #池化操作
